...
from lstm_risk_model import create_lstm_risk_model # Import the new model
...
def make_forward_looking_dataset_gem(df, events, features, is_lstm=False, timesteps=15):
    df_features = df.copy()
    ...
    # Create the forward-looking target Y
    y = pd.Series(0.0, index=df.index)
    for event in events:
        event_mask = (df['t'] >= event['start']) & (df['t'] < event['failure_time'])
        event_indices = df.index[event_mask]
        if len(event_indices) > 0:
            ramp = np.linspace(0, 1.0, len(event_indices))
            # MODIFIED: Use .loc for label-based indexing
            y.loc[event_indices] = ramp

    # Create X, Y, t for the model
    X, Y, t = [], [], []

    # Use -1 in range to align with Y, which is one step ahead
    ...
    for i in range(start_index, len(df_features)):
        ...
        # Y corresponds to the state at the end of the window
        Y.append(y.iloc[i - 1])
        t.append(df_features['t'].iloc[i - 1])

    return (
        np.array(X, dtype='float32'),
        np.array(Y, dtype='float32').reshape(-1, 1),
        np.array(t),
    )
...
def main():
    ...
    ap.add_argument('--timesteps', type=int, default=15, help="Lookback window for LSTM.")
    ...
    # --- 2. Evaluate LEAD-Drift (MLP with New Target) ---
    print("Training and evaluating LEAD-Drift (MLP)...")
    X_train_mlp, Y_train_mlp, t_train_mlp = make_forward_looking_dataset(df_train, train_events, features)
    X_test_mlp, _, t_test_mlp = make_forward_looking_dataset(df_test, test_events, features)
