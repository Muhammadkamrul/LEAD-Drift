...
def make_plots(df, outdir, window, tick_size, legend_size, label_size,
               fig_width, fig_height, dpi, min_size=None, max_size=None):
    ...
    # NEW: filter by dataset_size range (inclusive)
    if (min_size is not None) or (max_size is not None):
        mask = pd.Series(True, index=df.index)
        if min_size is not None:
            mask &= df["dataset_size"] >= min_size
        if max_size is not None:
            mask &= df["dataset_size"] <= max_size
        df = df[mask]
        if df.empty:
            raise ValueError(f"No rows after filtering dataset_size in [{min_size}, {max_size}].")
    ...
    for m in methods:
        ...
        if(m == "LEAD-Drift (MLP)" ):
            m = "LEAD-Drift"
        elif (m == "Baseline 3 (Weighted KPIs)" ):
            m = "Weighted-KPI heuristic baseline"
        elif (m == "Baseline 4 (Paper's Method)" ):
            m = "Distance-based baseline"
...
def main():
    ...
    make_plots(
        ...
        dpi=args.dpi,
        min_size=args.min_size,   # NEW
        max_size=args.max_size,   # NEW
    )
